...
Set your directory now to {\tt nki}.
...
The object we have created is a \textit{data frame} representation of the data in the file with 24481 rows (genes) and 296 columns (samples). We can now try the square bracket notation "[" to subset the rows and columns of the object.
...
\exercise Read the feature annotation for the dataset, which can be found in the file {\tt NKI295.fdata.txt}. Again, what are the dimensions are how to they correspond to the expression values?
