...
Set your directory now to {\tt bioc-intro}.
...
\exercise Use the \Rfunction{file.choose} function to locate the file {\tt NKI295.exprs.txt} and save the location as a variable.
...
Each row in this new matrix contains the expression values for a particular (arbitrary) gene for each patient in the study. It is worth noting that we could have used the \Rcode{row.names} argument to \Rfunction{read.delim} to pick a particular column in the original text file to be used as row names. This avoids having to rename the rownames after having read the file and creating a new copy of the data frame without the names.
...
\exercise Use the \Rfunction{rainbow} function to create a vector of length 295, and use this as an argument to change the colours in the scatter plot
<<>>=
...
The ExpressionSet is a complex object type.....Fortunately, few people actually have to fully-understand how it is implemented. Instead the authors provide a set of 'convenience functions', whose purpose is to access the data in the object in a painless manner. 
...
\exercise Use the \Rfunction{rowttests} function to calculate t-statistics for each gene, and then order the output.
...
\exercise Use a {\tt for} loop to make boxplots of the top 10 genes and save to a pdf file. Put the Symbol of each gene in the plot title.
